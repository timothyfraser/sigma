---
title: "Workshop 9: Statistical Process Control in `R`"
...
```{r, out.width = "100%", echo = FALSE, fig.cap="Statistical Process Control!"}
knitr::include_graphics("images/9_chart.png")
```
...
```{r, out.width="100%", echo = FALSE, fig.cap="Obanazawa City, Yamagata Prefecture - A Hot Springs Economy. [**Photo credit and more here.**](https://www.thehiddenjapan.com/ginzanonsen)"}
knitr::include_graphics("https://static.wixstatic.com/media/e8dc42_9cf1b5c0e5664ebd96581ffa8891eae2~mv2_d_1960_1200_s_2.jpg/v1/fill/w_1225,h_819,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/DSC00255-1-3_JPG.jpg")
...
```{r, fig.cap="Monkeys are even fans of onsen! Read [**more here!**](https://www.nytimes.com/2018/04/03/science/japan-monkeys-hot-springs-stress.html)", echo = FALSE, out.width = "100%"}
knitr::include_graphics("images/9_monkeys.png")
```
...
```{r sample_data, echo = FALSE, eval = FALSE}
...
tibble(time = 1:8,
  ...
  summarize(
    ...
    temp = case_when(
      time %in% c(1, 2, 3, 5) ~ rnorm(n = n, mean = 45, sd = 2),
      TRUE ~  rnorm(n = n, mean = 45, sd = 2) - time * rnorm(n = n, mean = 0.1, sd = 0.1)),
    
    ph = case_when(
      
      time %in% c(1, 3, 5, 7) ~ rnorm(n = n, mean = 5.5, sd = 0.5),
      TRUE ~ rnorm(n = n, mean = 5.5, sd = 0.5) - rnorm(n = n, mean = 0.1, sd = 0.1)),
    
    sulfur = case_when(
      time %in% c(1, 2, 3, 4, 5) ~ rgamma(n = n, shape = 0.5, rate = 1),
      TRUE ~ rgamma(n = n, shape = 0.5, rate = 1) + time / 2 * rnorm(n = n, mean = 0.1, sd = 0.1))) %>%
  ungroup() %>%
  mutate(temp = round(temp, 1),
         ...
         time = time * 2 - 1,
         id = 1:n()) %>%
  select(id, time, temp, ph, sulfur) %>%
  write_csv("/cloud/project/workshops/onsen.csv")
```
...
water = read_csv("/cloud/project/workshops/onsen.csv")
# Take a peek!
water %>% glimpse()
```
...
## 2.3 Total Statistics (Between Groups)
...
## 3.2 Individual and Moving Range Charts
...
## 3.4 Estimating $\sigma_{short}$ for Moving Range Statistics
