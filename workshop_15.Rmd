---
title: 'Workshop 15: Response Surface Methodology in `R`'
...
expand_grid(
  ...
  flour = c(2.75, 3, 3.25)) %>%
  get_data(base = 20) %>%
  write_csv("/cloud/project/workshops/gingerbread_test3.csv")


read_csv("/cloud/project/workshops/gingerbread_test3.csv") %>%
  filter(cinnamon == 1,
         butter == 1,
         flour == 3) %>%
  filter(molasses %in% c(0.5, 0.75, 1, 1.25),
         ginger %in% c(0.5, 1, 1.5, 2)) %>%
  write_csv("/cloud/project/workshops/gingerbread_test1.csv")

read_csv("/cloud/project/workshops/gingerbread_test3.csv") %>%
  filter(cinnamon == 1,
         butter == 1,
         flour == 3) %>%
  write_csv("/cloud/project/workshops/gingerbread_test2.csv")
...
```{r}
# Import our data
cookies = read_csv("/cloud/project/workshops/gingerbread_test1.csv")
# Check it out!
cookies %>% glimpse()
```
...
## 1.1 Specifying an Interaction Model with Polynomials
...
Suppose we expanded our factorial experiment based on this contour plot, adding more permutations of `molasses` and `ginger`, such that we now have `1280` cookies under test! We've saved this data in `/cloud/project/workshops/gingerbread_test2.csv`. 
...
```{r}
cookies2 = read_csv("/cloud/project/workshops/gingerbread_test2.csv")
```
...
Suppose now that we expanded our factorial experiment to vary the amount of `flour`, `butter`, and `cinnamon` too! We've saved this data in `/cloud/project/workshops/gingerbread_test3.csv`. How would we model this data?

```{r}
cookies3 = read_csv("/cloud/project/workshops/gingerbread_test3.csv")

cookies3 %>% glimpse()
```

## 3.1 Modeling many Interactions
...
## 3.2 Contours with Multiple Variables
...
# 4. Quantities of Interest in RSM
...
# 5. Extra Concepts of Interest: Canonical Form
...
## 5.1 Finding the Canonical Form
...
## 5.2 Canonical Form Coefficients
