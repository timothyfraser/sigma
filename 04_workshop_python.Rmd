...
```{python create-reliability-dataframe}
import numpy as np
...
```{python plot-life-distributions}
...
knitr::include_graphics("plotnine_figures/04_life_distributions.png")
```
...
```{python fplane-probability-calculations}
...
```{python fplane-failure-between-period}
...
```{python fplane-survival-past-6000}
...
```{python fplane-fleet-failure-projections}
...
```{python cup-multiplication-rule-examples}
...
```{python cup-multiplication-rule-5-cups}
...
```{python cup-complement-rule-2-cups}
...
```{python cup-complement-rule-5-cups}
...
```{python define-zcup-hazard-function}
...
```{python define-hcup-accumulative-hazard}
def hcup(days):
...
```{python define-afrcup-average-failure-rate}
def afrcup(t1, t2):
...
```{python define-afrcup-approximation}
