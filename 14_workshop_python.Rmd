# Factorial Design and Interaction Effects in `Python`

```{r setup_python_workshop_14, include=FALSE}
## Global options
...
lattes = pd.read_csv("https://raw.githubusercontent.com/timothyfraser/sysen/main/workshops/lattes.csv")
...
```{python}
# Let's make a dataframe 'groups'
# First group the lattes data by each combination of these variables
groups = lattes.groupby(['machine', 'milk', 'syrup', 'art'])

# Then add columns containing these summary statistics
groups = groups.agg(
  xbar = ("tastiness", "mean"),
  s = ("tastiness", "std"),
  n = ("tastiness", "count")
...
value
```
We can see that lattes made with Machine B and Torani syrup taste `r round(value, 2)` `r if (value < 0) "worse" else "better"`, on average, than lattes made with machine A and Monin syrup.
...
value
```
This `r if (value < 0) "negative" else "positive"` result indicates the combination of Machine type and Latte art `r if (value < 0) "dosen't" else "does"` improve tastiness when the syrup brand changes.
