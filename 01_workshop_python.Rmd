...
Notice: the IDE has four panes, including the editor, console, environment/history, and files. The console shows outputs from Python.
...
```
```{r image_py_1_2, echo = FALSE, out.width = "49%", fig.align='center', fig.cap="Save New Script!"}
...
```{r image_py_1_4, echo = FALSE, out.width = "100%", fig.cap = "Descriptive Stats Cheatsheet!"}
...
In Python we can use dfply's pipeline operator `>>` to connect data to functions. This reduces parentheses and keeps sequences readable.
...
We can visualize with `matplotlib`/`pandas` or use `plotnine` (a Python port of ggplot2).
