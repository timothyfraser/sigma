---
title: 'Workshop 12: Design of Experiments in R'
...
bind_rows(a1,a2,a3,b1,b2,b3) %>%
  write_csv("/cloud/project/workshops/donuts.csv")
```
...
donuts %>% glimpse()
```
# 1. Difference of Means (t-tests)

## 1.1 Calculating `dbar`
...
## 1.2 Visualizing the difference of means
...
# 2. Null vs. Sampling Distributions

But, how much should we *trust* our statistic $\bar{d}$ (`dbar`)? We can use (1) **null distributions** and (2) **sampling distributions** to characterize our level of trust in our statistic using (1) p-values and (2) confidence intervals, respectively.
...
## 3.1 Permutation Test (Computational Approach)
...
## 3.2 t-statistics (Theoretical Approach)
...
## 4.2 Assume Normal CIs for `dbar`
...
### 4.2.1  Paired-Sample/Dependent Sample T-Test ($n_1 = n_2$)
...
### 4.2.2 Unpaired-Sample/Independent Sample T-Test
...
Alternatively, `oneway.test()` can let you use `var.equal = TRUE or FALSE` to lightly adjust your statistics to accomodate uneven variances among groups. Randomization *should* usually deal with unequal variances, but if you're working with non-random samples, be sure to check your variances and then use `var.equal = FALSE`.

```{r}
# If even...
donuts %>%
  summarize(oneway.test(formula = weight ~ baker, var.equal = TRUE) %>% tidy())
```
