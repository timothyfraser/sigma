---
title: 'Workshop 13: Factorial Design and Interaction Effects in `R`'
...
knitr::include_graphics("/cloud/project/prep/images/12_lattes.jpg")
```
...
expand_grid(
  ...
  write_csv("/cloud/project/workshops/lattes.csv")
```
...
lattes = read_csv("/cloud/project/workshops/lattes.csv")
# or 
...
## 1.1 Difference of Grand Means $\bar{\bar{d}}$
...
## 1.2 Slower-but-Clearer Method of Getting the Difference of Grand Means $\bar{\bar{d}}$
...
## 1.3 Faster-but-Messier Method of Getting the Difference of Grand Means $\bar{\bar{d}}$
...
## 1.4 Estimating *Many* Difference of Grand Means $\bar{\bar{d}}$
...
# 2. Standard Errors for $\bar{\bar{d}}$
...
## 2.1 Estimating Standard Error when $s^2_i$ is known
...
## 2.1 Using Pooled Variance to Estimate Standard Errors
...
# 3. Pivoting Data with `pivot_longer()` & `pivot_wider()`
...
# 4. Visualizing & Reporting Treatment Effects
...
## 4.1 Building a Table of Direct Effects
...
## 6.2 Using `predict()` to visualize interaction effects
...
## 6.3 Finding the best model with `anova()`
