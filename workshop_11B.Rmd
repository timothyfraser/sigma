---
title: 'Workshop 11B: Multivariate Regression: Modeling Effects of Disaster on Social Capital'
...
read_csv("/cloud/project/workshops/japan_muni_elections.csv") %>%
  filter(year >= 2011) %>%
  ...
  write_csv("/cloud/project/workshops/jp_matching_experiment.csv")
```
...
# 0. Getting Started {.tabset .tabset-fade .tabset-pills}
...
# Load Data
cities <- read_csv("/cloud/project/workshops/jp_matching_experiment.csv") %>%
...
# 1. Multiple Regression {.tabset .tabset-fade .tabset-pills}
...
# LC 1 {.tabset .tabset-fade .tabset-pills}
...
# LC 2 {.tabset .tabset-fade .tabset-pills}
...
- Examine our two tables, using the ```texreg``` package's ```htmlreg()``` function. We're going to ```list()``` our models ```m1``` and ```m2```, and ask R to save a nice table in our files as ```"table_1.html"```. Try it out, then go to your files in the right-hand corner and click 'View in Web Browswer'! 

```{r, eval = FALSE}
htmlreg(list(m1,m2), 
        bold = 0.05, include.fstat = TRUE, 
        file = "/cloud/project/workshops/workshop_11_table_1.html")
```
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
htmltools::includeHTML(path = "/cloud/project/workshops/workshop_11_table_1.html")
```
...
# LC 3 {.tabset .tabset-fade .tabset-pills}
...
htmlreg(list(m1,m2,m3,m4), 
        bold = 0.05, include.fstat = TRUE, 
        file = "workshop_11_table_2.html")
```

<details><summary>**[Click to view table!]**</summary>

```{r, message = FALSE, warning = FALSE, echo = FALSE}
htmltools::includeHTML(path = "workshop_11_table_2.html")
```
...
htmlreg(
  ...
  bold = 0.05, 
  include.fstat = TRUE,
  file = "/cloud/project/workshops/workshop_11_table_3.html",
  # Add column labels
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
htmltools::includeHTML(path = "/cloud/project/workshops/workshop_11_table_3.html")
```
...
# LC 4 {.tabset .tabset-fade .tabset-pills}
...
htmlreg(
  ...
  bold = 0.05, 
  include.fstat = TRUE,
  file = "/cloud/project/workshops/workshop_11_table_4.html",
  custom.model.names = c(
    "Basic Model", "with Controls", 
    "With Extended<br>Controls", 
    "With Geographic<br>Controls"),
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
htmltools::includeHTML(path = "/cloud/project/workshops/workshop_11_table_4.html")
```
