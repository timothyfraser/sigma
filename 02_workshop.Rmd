...
# Check if PearsonDS is available, otherwise use alternative method
if (requireNamespace("PearsonDS", quietly = TRUE)) {
  ...
  # Fallback: use normal distribution with different means to show variation
  exlong <- data.frame(
    center = rnorm(1000, mean = stat_mean, sd = sqrt(stat_var)),
    right = rnorm(1000, mean = stat_mean + 1, sd = sqrt(stat_var)),
    left = rnorm(1000, mean = stat_mean - 1, sd = sqrt(stat_var))) %>%
    tidyr::pivot_longer(cols = -c(), names_to = "type", values_to = "x") %>%
    bind_rows(data.frame(type = "obs", x = sw)) %>%
    mutate(type = type %>% recode_factor(
      "obs" = "Observed",
...
if (requireNamespace("PearsonDS", quietly = TRUE)) {
  ...
  # Fallback: use normal distributions with different variances to show kurtosis variation
  exlong <- data.frame(
    medium = rnorm(1000, mean = stat_mean, sd = sqrt(stat_var)),
    low = rnorm(1000, mean = stat_mean, sd = sqrt(stat_var) * 1.2),
    high = rnorm(1000, mean = stat_mean, sd = sqrt(stat_var) * 0.8)) %>%
    tidyr::pivot_longer(cols = -c(), names_to = "type", values_to = "x") %>%
    bind_rows(data.frame(type = "obs", x = sw)) %>%
    mutate(type = type %>% recode_factor(
      "obs" = "Observed",
